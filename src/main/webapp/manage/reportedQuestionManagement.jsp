...
<head>
    ...
    <title>Reported Question Management</title>
...
<div class=" ml-64 my-10 px-8 py-8">
    ...
    <div class="flex space-x-6">
        <div>
            <div class="text-2xl font-semibold">Reported questions</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-md px-10 py-2 shadow-md mt-6">
            ...
            <%
                try {
                    ...
                    for (ReportedQuestionDTO reportedQuestion : list) {
                        String url = "/MainController?action=ViewReportedQuestion&reportedQuestionId="+reportedQuestion.getId();
            %>
            <tr onclick="window.location='<%=url%>';" class="cursor-pointer">
                <td class="px-5 py-2.5 border-b border-gray-200 bg-white text-sm text-center">
                    <p class="text-gray-900 whitespace-no-wrap">
                        <%=reportedQuestion.getId()%>
                    </p>
                </td>
                <td class="px-5 py-2.5 border-b border-gray-200 bg-white text-sm text-left">
                    <p class="text-gray-900 whitespace-no-wrap font-semibold">
                        <%= (70 > reportedQuestion.getDescription().length()) ? reportedQuestion.getDescription() : reportedQuestion.getDescription().substring(0,70) + "..."%>
                    </p>
                </td>
                <td class="px-5 py-2.5 border-b border-gray-200 bg-white text-sm text-center">
                    <p class="text-gray-900 whitespace-no-wrap">
                        <%=reportedQuestion.getOwnerUserId()%>
                    </p>
                </td>
                <td class="px-5 py-2.5 border-b border-gray-200 bg-white text-sm text-center">
                    <p class="text-gray-900 whitespace-no-wrap">
                        <%=reportedQuestion.getCreationDate()%>
                    </p>
                </td>
                <td class="px-5 py-2.5 border-b border-gray-200 bg-white text-sm text-center">
                    <p class="text-gray-900 whitespace-no-wrap">
                        <%=listFlagType.get((int) reportedQuestion.getFlagTypeId()).getName()%>
                    </p>
                </td>
                </a>
            </tr>
        ...
        <div class="pagination p-3 inline-flex space-x-3 sm:ml-20 ml-12">
            <a href="MainController?action=ReportedQuestion&page=<%=previousPage%>" class="relative px-2 py-2 rounded border border-purple-600 text-sm font-medium text-gray-500 hover:bg-gray-50">
            ...
            <a href="MainController?action=ReportedQuestion&page=<%=nextPage%>" class="relative px-2 py-2 rounded border border-purple-600 text-sm font-medium text-gray-500 hover:bg-gray-50">
